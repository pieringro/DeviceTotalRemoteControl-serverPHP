<?php
	 ...
	 * listing a collection of DtrcDevices objects.  It includes
	 ...
	 * 
	 */
	class DtrcDevicesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this DtrcDevices datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DtrcDevices object, or it can be any QQNode extending from QQN::DtrcDevices().
		 * 
		 ...
		 * sorting using two fields from the DtrcDevices object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DtrcDevices
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?strId=<?=urlencode($_ITEM->Id)?>&strEmailUser=<?=urlencode($_ITEM->EmailUser)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->Id))?>/<?=str_replace("+","%20",urlencode($_ITEM->EmailUser))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->Id . "," . $_ITEM->EmailUser, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = DtrcDevices::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from DtrcDevices, given the clauses above
			$this->DataSource = DtrcDevices::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DtrcDevices-based QQNode.
		 * It will also verify that it is a proper DtrcDevices-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'dtrc_devices') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "dtrc_devices".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::DtrcDevices()->Id;
				case 'EmailUser': return QQN::DtrcDevices()->EmailUser;
				case 'EmailUserObject': return QQN::DtrcDevices()->EmailUserObject;
				case 'TokenFirebase': return QQN::DtrcDevices()->TokenFirebase;
				default: throw new QCallerException('Simple Property not found in DtrcDevicesDataGrid content: ' . $mixContent);
