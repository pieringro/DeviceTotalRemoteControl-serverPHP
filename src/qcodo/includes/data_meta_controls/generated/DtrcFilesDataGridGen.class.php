<?php
	 ...
	 * listing a collection of DtrcFiles objects.  It includes
	 ...
	 * 
	 */
	class DtrcFilesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this DtrcFiles datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DtrcFiles object, or it can be any QQNode extending from QQN::DtrcFiles().
		 * 
		 ...
		 * sorting using two fields from the DtrcFiles object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DtrcFiles
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = DtrcFiles::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from DtrcFiles, given the clauses above
			$this->DataSource = DtrcFiles::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DtrcFiles-based QQNode.
		 * It will also verify that it is a proper DtrcFiles-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'dtrc_files') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "dtrc_files".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::DtrcFiles()->Id;
				case 'IDDevices': return QQN::DtrcFiles()->IDDevices;
				...
				case 'Length': return QQN::DtrcFiles()->Length;
